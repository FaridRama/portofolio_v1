...
const Card = ({ children, className = "" }) => (
  <div className={`relative group ${className}`}>
    <div className="absolute -inset-0.5 bg-gradient-to-r from-gray-500 to-white rounded-2xl blur opacity-10 group-hover:opacity-20 transition duration-500 pointer-events-none" />
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start sm:items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          <div className="relative">
            <div className="absolute -inset-0.5 bg-gradient-to-r from-gray-500 to-white rounded-xl blur opacity-50 pointer-events-none" />
          ...
          {[
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 sm:px-4 py-1.5 rounded-lg text-xs sm:text-sm transition-all duration-200 ${
                filter === tab.value
                  ? "bg-gradient-to-r from-white/15 to-gray-500/10 border border-white/30 text-white font-medium"
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full bg-white/5 border border-white/10 rounded-xl pl-10 pr-10 py-2.5 text-gray-200 placeholder-gray-600 text-sm outline-none focus:border-white/40 focus:ring-1 focus:ring-white/20 transition-all"
        />
        {search && (
          <button
            onClick={() => setSearch("")}
        ...
        <div className="space-y-2.5">
          {paginated.map((comment) => (
            <div key={comment.id} className="relative group">
              {comment.is_pinned && (
                <div className="absolute -inset-0.5 bg-gradient-to-r from-gray-500 to-white rounded-2xl blur opacity-15 pointer-events-none" />
              )}
              <div
              ...
              >
                <div className="flex items-start gap-3 sm:gap-4">
                  {/* Avatar */}
                  <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-white/10 to-gray-500/10 border border-white/10 flex items-center justify-center shrink-0">
                    <img
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      ...
                      {comment.is_pinned && (
                        <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-white/10 border border-white/20 text-gray-300 text-xs">
...
function highlightMatch(text, query) {
  ...
  return parts.map((part, i) =>
    regex.test(part) ? (
      <mark key={i} className="bg-white/20 text-white rounded px-0.5">
        {part}
      </mark>
    ) : (
      part
