...
const CertCard = ({ cert, onDelete }) => {
  ...
  return (
    <div className="relative group">
      <div className="absolute -inset-0.5 bg-gradient-to-r from-gray-500 to-white rounded-2xl blur opacity-10 group-hover:opacity-30 transition duration-500" />
      <div className="relative bg-white/5 border border-white/12 rounded-2xl overflow-hidden">
...
export default function Certificates() {
  ...
  const uploadImage = async () => {
    ...
    try {
      ...
      const { data: urlData } = supabase.storage.from('certificate-images').getPublicUrl(fileName)
      const { error: insertError } = await supabase.from('certificates').insert({ img: urlData.publicUrl })
      ...
      alert("Gagal upload sertifikat: " + error.message)
    } finally {
      setUploading(false)
    }
  }
            ...
            <div className="flex items-center justify-between gap-3 flex-wrap">
              ...
              <div className="flex gap-2 shrink-0">
                ...
                <button onClick={uploadImage} disabled={uploading} className="relative group/u">
                  <div className="absolute -inset-0.5 bg-gradient-to-r from-gray-600 to-white rounded-xl opacity-60 blur group-hover/u:opacity-100 transition duration-300" />
                  <div className="relative flex items-center gap-2 px-4 py-1.5 bg-[#000000] rounded-xl border border-white/10">
